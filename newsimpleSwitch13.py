...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        self.logger.info("Switch %s connected", switch_id)
    ...
    def _packet_in_handler(self, ev):
        ...
        if self.edge_switch(switch_id):
            ...
            if src_host_id == switch_id:
                ...
                self.push_mpls_flow(datapath, priority=20, match=match, mpls_label=mpls_label, mpls_tc=mpls_tc, mpls_ttl=mpls_ttl, out_port=out_port)#out_port)


                return
            else:
                print("Egress")
                self.pop_mpls_flow(datapath, priority=10, match=match, out_port=out_port)
                 #self.pop_mpls_flow
                return
