...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        if dst in self.net:
            path=nx.all_simple_path(self.net,src,dst) # get shortest path  
        ...
        for src_dpid, dst_dpid, link_info in links:
            ...
            switch_ports[src_dpid] = {'in_port': [], 'out_port': []}

        if dst_dpid not in switch_ports:
            switch_ports[dst_dpid] = {'in_port': [], 'out_port': []}

         # Add the input and output ports for each switch
        switch_ports[src_dpid]['out_port'].append(link_info['port'])
        switch_ports[dst_dpid]['in_port'].append(link_info['port'])
